...
- **最近一次 bundle 健康验证**：2026-06-09 本地重打包后，`./dev_checks.py packaged` 为 12 项就绪、1 项留意（ad-hoc 签名）、0 项未就绪（新增摄像头占用探针检查）。
- Linux/Windows：不在目标内。
...
- 摄像头占用信号已接入自动免打扰（AVFoundation 只读设备状态），无摄像头开发机上已验证优雅降级；当前更适合继续做的是真实证书/Apple 凭据下的签名公证实机闭环、带摄像头真机的占用实测，以及首次运行里的权限解释细节继续打磨。

### 阶段六：质量升级（后续）
- 升级到更强模型（只改后端配置）。
- 美术升级到 Live2D（只改底层渲染层）：头部跟随、视线追踪、头发/裙摆/纱物理摆动、口型同步等。需分层 PSD + Cubism 建模，作为独立升级项。
- 打包 / 公证、预算解释层继续细化、立绘成品替换仍在后续范围内。
...
| 免打扰 / 场景感知 | ✅ 已接手动免打扰 + 忙时延后提醒 + 自动免打扰基线（全屏 / 会议 / 共享标题 / 系统 Focus / 原生事件刷新）+ 自动隐藏 + 更直接的共享/录屏窗口识别 + 可持久化检测总开关 / 状态回执 + 摄像头占用信号（AVFoundation 只读设备状态，不采集、不触发摄像头 TCC；命中只静音不隐藏）；剩余为真机摄像头实测 |
| 饥饿系统（预算驱动） | ✅ 已接上：按日/周预算消耗比例切 peckish(80%) / hungry(95%) / starving(100%)，blocked 直接按断粮；台词全脚本化不走 API、阈值向上越过播报一次、回血与重置回执合并报喜；petdex 包 waiting / failed / jumping 帧充当讨食 / 虚弱 / 庆祝形态，缺形态的包逐级降级 |
| 首次运行引导 | ✅ 已接较完整基础版：打包版首启 API key / 命名 / 预算 / 数据边界确认 / 自动隐藏 / 权限自检入口，并已开始跟随系统 / UI 语言 |
| 打包 / `.app` TCC 自检 / 公证 | 🟡 已接真实 `.app` 构建 + bundle 内健康验证 + Launch Services 启动验证 + 固定实机回归清单；权限恢复提示、codesign/notarytool/stapler/Gatekeeper 命令链也已补上，最近一次 bundle 内健康验证为 12 项就绪、1 项留意（ad-hoc 签名）、0 项未就绪；真实证书/Apple 凭据下的实机闭环与更多 TCC 组合场景待补 |
...
2. **§16 自动免打扰剩余场景**：全屏 / 会议 / 共享标题 / 系统 Focus / 更直接的共享与录屏高信号 / 摄像头占用信号都已接上；开发机（Mac mini）无摄像头，摄像头信号还需在带摄像头的真机上实测一次。
...
- **自动场景感知（摄像头已接）**：检测到**摄像头被其他应用占用**时（AVFoundation `isInUseByAnotherApplication`，只读设备状态、不采集画面、不触发摄像头 TCC 弹窗），自动进入免打扰，优先级介于显式共享信号与系统 Focus 之间。注意：摄像头占用只静音、不自动隐藏立绘——摄像头开着不代表屏幕被人看见，隐藏仍只跟共享/录屏信号走。
- **免打扰下的行为**：免打扰期间，提醒/吐槽/饥饿提示全部静默或排队（呼应 §6.1「不打断主人」的仲裁规则），等场景结束再说。高优先级的权限请求（§6.2）可设例外，但默认也应延后。
- **作息感知（可选）**：深夜可自动降低主动发言频率，别半夜蹦出来吓人。

> 实现归属：第 1 层。macOS 上检测屏幕共享/录屏/摄像头状态需要 PyObjC 调系统 API，归到 §5 的 Mac 专属窗口模块一并处理。当前“手动 DND + 提醒延后 + 自动免打扰基线 + 自动隐藏 + 更直接的共享/录屏窗口信号 + 检测总开关 / 状态回执 + 摄像头占用信号”都已落地；摄像头信号在无摄像头的开发机上只验证了优雅降级，还需带摄像头真机实测一次。
...
| B3 | **重启 / 睡眠后状态延续** | SDK 会话 resume 已落盘；预算窗重置提示与睡眠折算基础版也已接上；饥饿进度天然随预算账本持久化（重启按当前消耗直接算档位、不重播旧播报）；窗口位置也已落地——立绘显示设置里以「屏幕可用区百分比 + 目标屏模式」存进 `app_state`（`sprite_position_x/y_percent`），启动时 `move_to_saved_start_position` 按当前分辨率还原，换屏/改分辨率不丢位。剩余：自由拖拽后的落点尚未自动回写（只持久化设置里配的起始位），以及更细的长驻状态折算。 | 阶段五 / 六 |
...
| B7 | **免打扰 / 场景感知补完** | 手动 DND、全屏/会议/共享标题/系统 Focus 自动 DND、自动隐藏、更直接的共享/录屏窗口信号、可持久化检测总开关 / 状态回执、以及摄像头占用信号都已接上；剩余是在带摄像头的真机上实测一次占用检测。 | 上线前 |
